...
class OptimizedMedicalQASystem:
    ...
    def load_knowledge_base(self):
        ...
        try:
            ...
            logger.info(f"成功加载知识库: {len(self.knowledge_base)} 个知识块")
    ...
    def intelligent_retrieve(self, query: str, top_k: int = 10) -> List[Dict]:
        ...
        for expanded_query in expanded_queries:
            candidates = self._retrieve_for_query(expanded_query, top_k // len(expanded_queries) + 2)
    ...
    def _calculate_chunk_relevance(self, chunk: Dict, query: str, query_words: set, query_clean: str) -> float:
        ...
        for term in medical_terms:
            ...
            if term_lower in query_clean:
                term_matches += 1
            elif query_clean in term_lower:
                term_matches += 1
        
        if medical_terms:
            term_score = (term_matches / len(medical_terms)) * 0.15
            score += term_score
        ...
        if content_words:
            overlap_score = (overlap / max(len(query_words), len(content_words))) * 0.05
            score += overlap_score
    ...
    def _rank_candidates(self, candidates: List[Dict], original_query: str) -> List[Dict]:
        ...
        for candidate in candidates:
            ...
            final_score = relevance + chapter_bonus + content_bonus
    ...
    def generate_response_with_citations(self, query: str, candidates: List[Dict]) -> Dict:
        ...
        # 开头引导语
        if query_type == 'what':
            answer_parts.append(f"根据医学资料，关于{core_concept}的定义和概念：")
        elif query_type == 'how':
            ...
            answer_parts.append(f"根据医学资料检索结果：")
    ...
    def generate_precise_citation(self, chunk: Dict) -> Dict:
        ...
        citation = {
            'source_file': metadata.get('document_title', '医学教材'),
            'page_number': metadata.get('page_number', 'N/A'),
            'chapter': chunk.get('chapter', 'N/A'),
            'section': chunk.get('section', 'N/A'),
            'knowledge_chain_id': chunk.get('id', 'N/A'),
            'content_preview': chunk.get('content', '')[:100] + '...' if len(chunk.get('content', '')) > 100 else chunk.get('content', ''),
            'medical_terms': chunk.get('medical_terms', [])[:5],  # 只显示前5个术语
            'line_numbers': metadata.get('line_numbers', []),
            'relevance_score': 0.0
        }
        
        # 添加知识类型信息
        if 'knowledge_patterns' in chunk:
            citation['knowledge_types'] = chunk['knowledge_patterns']
    ...
    def process_query(self, query: str, top_k: int = 10) -> Dict:
        ...
        response['statistics'] = {
            'total_candidates_found': len(candidates),
            'knowledge_chunks_processed': len(self.knowledge_base),
...
def main():
    """测试函数"""
    qa_system = OptimizedMedicalQASystem(
    ...
    for query in test_queries:
        ...
        if response['citations']:
            ...
            for i, citation in enumerate(response['citations'][:2], 1):
                print(f"  {i}. {citation['source_file']} - {citation['chapter']} - {citation['section']}")
